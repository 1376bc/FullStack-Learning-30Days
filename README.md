...
| **第三阶段** | Day 11-14 | 业务深化 | CRUD 闭环、分页搜索、多表关联、登录安全 | ✅ 已通关 |
| **核心跨越** | **Day 15** | **异构架构** | **Python 报表微服务 + 跨服务数据导出** | ✅ 已通关 |
| **第四阶段** | Day 16-30 | 项目成品化 | 文件上传、部署上线、系统监控 | ⏳ 进行中 |
...
## 📅 Day 15：异构多服务架构与数据报表 ✅
### 1. 核心技术突破
- **多语言协同**: 实现了 Java (OLTP) 与 Python (OLAP) 的解耦运行。Java 负责实时 CRUD，Python 负责离线报表。
- **SQL 深度应用**: 掌握了 `LEFT JOIN` 与 `GROUP BY` 的组合查询，解决了“空班级人数为 0”的统计难题。
- **自动化流转**: 实现了 `Database -> SQLAlchemy -> Pandas -> Excel -> FastAPI -> Browser` 的完整数据导出链路。

### 2. 避坑指南补遗
- **端口管理**: 运行多个后端时，必须注意端口冲突（Java 8080, Python 8000）。
- **CORS 跨域**: 在多服务环境下，每个后端（Java 和 Python）都必须显式开启跨域许可，否则前端无法同时调用。

### 3. 面试复盘
**Q：如果数据量达到百万级，你的 Python 报表脚本会遇到什么问题？**
**A**：会产生内存溢出（OOM）。因为 `pd.read_sql` 默认一次性将所有数据读入内存。优化方案是：使用 SQL 分批读取（Chunking），或者直接在数据库层完成复杂的聚合运算，Python 只负责最后的格式化和文件输出。
----
## 📝 技术心得汇总

### 1. 异构系统架构 (Day 15 重点)
- **多语言解耦**: 实现了 Java (负责实时业务 CRUD) 与 Python (负责离线数据分析) 的并行运行。
- **微服务雏形**: 前端 HTML 作为“指挥官”，同时调用 8080 (Java) 和 8000 (Python) 两个端口，理解了模块化设计的优势。
- **数据流转**: 掌握了从 `MySQL -> SQLAlchemy -> Pandas -> Excel -> FastAPI` 的全自动化报表链路。

### 2. 后端开发 (Spring Boot / JPA)
- **ORM 关联映射**: 掌握了 `@ManyToOne` 建立学生与班级的 1:N 关系，并解决了 JSON 递归死循环问题。
- **标准 REST API**: 实现了带分页 (`Pageable`) 和模糊搜索的工业级接口。
- **数据验证**: 引入 `Validation` 起步依赖，实现后端数据强校验。

### 3. 系统安全与交互
- **身份认证**: 实现了基于数据库校验的登录流，并使用前端 `localStorage` 进行路由守卫。
- **异步体验**: 熟练应用 `fetch` + `async/await`，并引入 `SweetAlert2` 提升 UI 反馈质感。
...
### 🔴 开发环境与网络
1. **Git Proxy**: 遇到 `port 443` 超时，需执行 `git config --global --unset http.proxy` 或正确配置加速器端口。
2. **端口冲突**: 运行多个后端时需注意 8080 与 8000 的分配，遇到占用使用 `taskkill` 强杀 PID。
3. **CORS 跨域**: 多后端环境下，每个服务必须显式配置 `@CrossOrigin` 或中间件，否则前端调用会被拦截。

### 🟡 数据库与逻辑
1. **DB Selection**: 运行 SQL 脚本前必加 `USE database_name;`，避免 `No database selected` 错误。
2. **类型兼容性**: Java `Long` 对应 MySQL `BIGINT`。若外键类型不一致（如指向了 INT），会导致 DDL 运行失败。
3. **旧数据断层**: 增加多表关联后，需手动更新数据库旧记录的 `class_id`，否则前端会显示“无”。

### 🔵 代码细节
1. **引号陷阱**: JS 模板字符串 `${}` 必须使用 **反引号 (`` ` ``)**，使用单双引号会导致变量无法解析。
2. **JSON 结构**: 后端返回 `Page` 对象后，前端必须访问 `data.content` 才能拿到数组。
3. **拼写误差**: 变量定义为 `option` 却写成 `ooption`，此类错误通过浏览器 F12 控制台定位。

---

## 👨‍💻 面试官提问模拟 (Interview Q&A)

**Q1：为什么把报表功能交给 Python 而不是直接在 Java 里写？**
> **A**：这体现了 **OLTP (联机事务处理)** 和 **OLAP (联机分析处理)** 分离的思想。Java 擅长处理高并发的实时增删改查；而报表统计涉及大量聚合运算（GROUP BY, LEFT JOIN），使用 Python 的 Pandas 库能更高效地处理数据，且不会占用 Java 主服务的内存资源，避免业务卡顿。

**Q2：你在做多表关联时如何解决实体类循环引用导致的栈溢出？**
> **A**：在双向关联中，Jackson 序列化时会陷入死循环。我使用了 `@JsonIgnoreProperties` 忽略掉关联对象中的循环属性，或者使用 `@JsonManagedReference` 和 `@JsonBackReference` 显式标注主从关系，从而确保 JSON 结构的清晰。

**Q3：目前的登录系统安全性如何？你会如何改进？**
> **A**：目前实现了基础的前端守卫。改进方案是引入 **Spring Security + JWT**。后端不再保存状态，而是下发加密 Token。前端在请求头中携带 Token，后端拦截器通过解析 Token 来判定权限，这样即使黑客绕过前端页面直接调用接口也会被拦截。
